// app.js 상단에 다음 코드 추가 (기존 코드 대체)
class ProductionPlanner {
    constructor() {
        // Supabase 초기화 (사용자 제공 정보 사용)
        this.supabase = window.supabase.createClient(
            'https://kyspwjebzbozuzhgngxm.supabase.co',
            'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imt5c3B3amViemJvenV6aGduZ3htIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDg3MTk0MTUsImV4cCI6MjA2NDI5NTQxNX0.10iosLA08Q__Y7E6aJgtOWt5_AEYS783kHxSSXsf9Po'
        );
        ...
        this.supabase
            ...
            .on('postgres_changes', 
                ...
                (payload) => {
                    ...
                    this.loadPlansFromSupabase().then(() => {
                        this.renderCalendar();
                        this.renderPlansList();
                        this.showToast('새로운 변경사항이 동기화되었습니다', 'success');
                    });
                }
            )
            .subscribe();
    ...
    // 데이터 저장 로직 최적화
    async savePlanToSupabase(planData) {
    ...
    async loadPlansFromSupabase() {
        try {
            ...
            this.showToast('데이터 불러오기 실패', 'error');
        }
    }
    ...
    async deletePlan(planId) {
        try {
            ...
            this.showToast('계획이 삭제되었습니다', 'success');
        } catch (error) {
