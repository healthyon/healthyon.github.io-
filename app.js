...
class ProductionPlannerApp {
    ...
    openEditModal(id) {
        ...
        const plan = this.productionPlans.find(p => p.id === numericId);
        if (!plan) return;
        this.editTarget = numericId;
        // 수정용 폼에 기존 값 채우기
    ...
    async confirmEdit() {
        ...
        let orderQuantity = document.getElementById('editOrderQuantity').value.replace(/,/g, '');
        ...
        try {
            ...
            if (index !== -1) {
                this.productionPlans[index] = { id, ...updatedData, created_at: this.productionPlans[index].created_at };
                this.saveToLocalStorage();
                this.updateCalendar();
                this.updateProductionTable();
            }
    ...
    updateProductionTable() {
        ...
        tbody.innerHTML = this.productionPlans.map(plan => `
            <tr>
                ...
                <td>${plan.end_date}</td>
                <td>
                    <button class="btn btn--secondary btn--small" onclick="app.openEditModal('${plan.id}')">
                        수정
                    </button>
...
document.addEventListener('DOMContentLoaded', () => {
    // app 인스턴스를 전역 객체로 등록
    window.app = new ProductionPlannerApp();
});
